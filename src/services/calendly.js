...
class CalendlyService extends CloudApp {
  ...
  inviteUserToOrg(email) {
    this.setAuthorizationToken(process.env.CALENDLY_API_KEY);
    return this.create(`/organizations/${process.env.CALENDLY_ORG_ID}/invitations`, { email });
  }

  listOrganizationMembers() {
    this.setAuthorizationToken(process.env.CALENDLY_API_KEY);
    return this.read('/organization_memberships');
  }

  removeUserFromOrg(uuid) {
    this.setAuthorizationToken(process.env.CALENDLY_API_KEY);
    return this.delete(`/organization_memberships/${uuid}`);
  }
}
